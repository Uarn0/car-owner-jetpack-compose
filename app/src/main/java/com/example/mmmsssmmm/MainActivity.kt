...
import com.example.mmmsssmmm.eventModel.IEvent
...
import com.example.mmmsssmmm.vehicleModel.IVehicle
...
fun Main() {
    ...
    val vehicles = vehiclesDao.observeVehicles().collectAsState(initial = emptyList()).value
    ...
    NavHost(navController = navController, startDestination = "VehicleList") {
        ...
        composable("VehicleList") {
            MainList(
                vehicles = vehicles,
                onDetailsClick = { v -> navController.navigate("EventList/${v.id}") },
                onAddClick = { navController.navigate("AddVehicle") },
                onDelete = { v ->
                    scope.launch { vehiclesDao.delete(v) }
                }
            )
        }
        ...
        composable("EventList/{vehicleId}") { backStackEntry ->
            ...
            val events = eventDao.observeEvents(vehicleId).collectAsState(initial = emptyList()).value
            EventList(
                events = events,
                onAddClick = { navController.navigate("AddEvent/$vehicleId") },
                onDeleteClick = { e -> scope.launch { eventDao.delete(e) } }
            )
        }
        ...
        composable("AddEvent/{vehicleId}") { backStackEntry ->
            ...
            EventInput(onSave = { name, type, date ->
                scope.launch {
                    eventDao.insert(EventEntity(vehicleId = vehicleId, name = name, type = type, time = date))
                    navController.popBackStack()
                }
            })
        }
